...
CKEDITOR.on('dialogDefinition', function (ev) {
    ...
    if (dialogName == 'link') {
        ...
        var itemNames = ['drupal', 'url'];

        // @FIXME: Check browser support for Array.filter and simplify.
        linkOptions.items = $.grep(linkOptions.items, function (item) {
            return $.inArray(item[1], itemNames) > -1;
        });
    }
})
